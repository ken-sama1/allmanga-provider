const buildSelectionSet = (selection, indentLevel = 1) => {
    const indent = "  ";
    ...
    return `{${newLineWithIndent}${selectionSetString}\n${indentLevel > 1 ? indent.repeat(indentLevel - 1) : ""} }`.replace(/["]/g, "");
};
const buildVariableDefinition = (variables, types) => {
    const keysWithType = [];
...
export const buildQuery = (args) => {
    const { definitionString, variableString } = buildVariableDefinition(args.variables, args.types);
